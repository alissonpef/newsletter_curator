...
def test_run_history_creation():
    rh = RunHistory(status="success", statusLabel="Sucesso", checkpointLabel="Concluído")
...
def test_topic_creation():
    topic = Topic(title="Test Topic", summary="This is a summary", impact="High")
    assert topic.signal == "Médio"
...
def test_job_status_creation():
    js = JobStatus(dateRef="2023-10-01", status="running", statusLabel="Em andamento")
...
def test_daily_digest_creation():
    dd = DailyDigest(
        dateRef="2023-10-01", dateLabel="01 de Outubro", status="completed", statusLabel="Concluído"
    )
