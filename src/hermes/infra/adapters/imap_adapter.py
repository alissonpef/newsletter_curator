...
class ImapAdapter(EmailPort):
    def __init__(
        ...
        self.allowed_senders = [
            sender.strip().lower()
            for sender in str(allowed_senders).split(",")
            if sender.strip()
        ]

    def _build_search_queries(self, search_date: str) -> List[str]:
        next_date = (
            datetime.strptime(search_date, "%d-%b-%Y") + timedelta(days=1)
        ).strftime("%d-%b-%Y")
    ...
    def _extract_message_text(self, msg: Message) -> str:
        if msg.is_multipart():
            ...
            text = payload.decode(
                best_part.get_content_charset() or "utf-8", errors="ignore"
            )
        ...
        try:
            ...
            for query in self._build_search_queries(search_date):
                ...
                if status != "OK":
                    continue
                message_ids = [
                    message_id for message_id in messages[0].split() if message_id
                ]
                if message_ids:
                    break
            ...
            for email_id in message_ids:
                ...
                for response_part in msg_data:
                    ...
                    if (
                        self.allowed_senders
                        and sender_email not in self.allowed_senders
                    ):
                        continue
